...
class diff_vw: public Move_Robot
{
	...
	float Rev_V, Rev_W, Rev_a, Rev_th, Rev_x, Rev_y;
	int type;
...
{
		//std::cout<<"Rev_V = "<<Rev_V<<" Rev_W =  "<<Rev_W<<" Rev_x =  "<<Rev_x<<" Rev_y = "<<Rev_y<<"Rev_th = "<<Rev_th<<std::endl;
		...
		// std::cout<<"base_link_x = "<<odom_trans.transform.translation.x<<std::endl;
		// std::cout<<"base_link_y = "<<odom_trans.transform.translation.y<<std::endl;
...
void diff_vw::RevProcess(double receive_period)
{
    ...
    //std::cout<<"=========LOSS encoder========="<<std::endl;
		if(mySerial.serial_ok == true){
			...
			int readByte = 0;
			int rev_count = 0;
			readByte = read(mySerial.fd,buff,100);
			if(readByte > 0)
			{
			...
			if(rev_buf[0] == 'E' && rev_buf[1] == 'C' && rev_buf[rev_buf.size()-4] == 'C' && rev_buf[rev_buf.size()-3] == 'E')
			{
					std::string recv_data;
					std::string cut_par;
					std::string recv_pkg[5];
					int rev_count = 0;
					for(int i=3;i<rev_buf.size()-5;i++)
					{
							recv_data.push_back(rev_buf[i]);
					}
					//std::cout<<recv_data<<std::endl;
					std::stringstream cut(recv_data);
					while(getline(cut,cut_par,','))
					{
						recv_pkg[rev_count]=cut_par;
						//std::cout<<" recv_pkg= "<<rev_count << " "<< recv_pkg[rev_count]<<std::endl;
						rev_count++;
					}
					Rev_V = std::stof(recv_pkg[0]);
					Rev_W = std::stof(recv_pkg[1]);
					Rev_x = std::stof(recv_pkg[2]);
					Rev_y = std::stof(recv_pkg[3]);
					Rev_th = std::stof(recv_pkg[4]);
			}
			// if(rev_buf.size() == 11)
			// {
